...
const EmailEditForm = () => {
    ...
    const getUserById = async () => {
      ...
      try {
        const response = await axios.get(
          `http://172.25.160.235:2402/emailNotification/${id}`, config
        );
        setRecipientEmail(response.data.recipientEmail);
        setRecieverName(response.data.RecieverName);
      } catch (error) {
        if (error.response) {
  ...
  const updateUser = async (e) => {
    ...
    try {
      await axios.patch(`http://172.25.160.235:2402/emailNotification/${id}`, {
  ...
  const handleAuditEditStaff = async () => {
    ...
    const action = `Edit email info ${recipientEmail} ${RecieverName}`;
